...
{
    [Serializable, VolumeComponentMenu("Post-processing/Kino/GenshinBloom")]
    public sealed class GenshinBloom : CustomPostProcessVolumeComponent, IPostProcessComponent
    {
        ...
        Dictionary<int, GenshinBloomPyramid> _pyramids;

        GenshinBloomPyramid GetPyramid(HDCamera camera)
        {
            GenshinBloomPyramid candid;
            ...
            else
            {
                ...
                _pyramids[cameraID] = candid = new GenshinBloomPyramid(camera);
            }
        ...
        {
            _material = CoreUtils.CreateEngineMaterial("Hidden/Kino/PostProcess/GenshinBloom");
            _prop = new MaterialPropertyBlock();
            _pyramids = new Dictionary<int, GenshinBloomPyramid>();
        }
        ...
        public override void Render(CommandBuffer cmd, HDCamera camera, RTHandle srcRT, RTHandle destRT)
        {
            ...
            HDUtils.DrawFullScreen(cmd, _material, pyramid[0].A, _prop, 0);
            // upにも書き込む
            ...
            for (; level < GenshinBloomPyramid.MaxMipLevel && pyramid[level].A != null; level++)
            {
                ...
                HDUtils.DrawFullScreen(cmd, _material, pyramid[level].B, _prop, 8);
                _prop.SetTexture(ShaderIDs.InputTexture, pyramid[level].B);
                HDUtils.DrawFullScreen(cmd, _material, pyramid[level].A, _prop, 9);
            ...
            _prop.SetTexture(ShaderIDs.BloomTextureA, pyramid[0].A);
            _prop.SetTexture(ShaderIDs.BloomTextureB, pyramid[1].A);
            _prop.SetTexture(ShaderIDs.BloomTextureC, pyramid[2].A);
            _prop.SetTexture(ShaderIDs.BloomTextureD, pyramid[3].A);

            // _prop.SetTexture(ShaderIDs.InputTexture, pyramid[3].A);
            HDUtils.DrawFullScreen(cmd, _material, pyramid[0].B, _prop, 11);
            _prop.SetTexture(ShaderIDs.BloomTextureA, pyramid[0].B);
    ...
    sealed class GenshinBloomPyramid
    {
        public const int MaxMipLevel = 16;

        int _baseWidth, _baseHeight;
        readonly (RTHandle A, RTHandle B) [] _mips = new (RTHandle, RTHandle) [MaxMipLevel];

        public (RTHandle A, RTHandle B) this [int index]
        {
            get { return _mips[index]; }
        }

        public GenshinBloomPyramid(HDCamera camera)
        {
            Allocate(camera);
        }
        ...
        public void Release()
        {
            ...
            {
                if (mip.A != null) RTHandles.Release(mip.A);
                if (mip.B != null) RTHandles.Release(mip.B);
            }
        }
