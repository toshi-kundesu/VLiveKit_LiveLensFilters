...
{
    [Serializable, VolumeComponentMenu("Post-processing/Kino/GenshinColorGrading")]
    public sealed class GenshinColorGrading : CustomPostProcessVolumeComponent, IPostProcessComponent
    {
        ...
        /* ─────────── パラメータ ─────────── */
        public ClampedFloatParameter exposure   = new(1f, 0f, 5f);
        public ClampedFloatParameter contrast   = new(1f, 0f, 2f);
        public ClampedFloatParameter saturation = new(1f, 0f, 2f);
        public BoolParameter         toneMap    = new(false);   // 独自トーンマップを使う
        ...
        Dictionary<int, GenshinColorGradingPyramid> _pyramids;

        GenshinColorGradingPyramid GetPyramid(HDCamera camera)
        {
            GenshinColorGradingPyramid candid;
            ...
            else
            {
                ...
                _pyramids[cameraID] = candid = new GenshinColorGradingPyramid(camera);
            }
        ...
        {
            _material = CoreUtils.CreateEngineMaterial("Hidden/Kino/PostProcess/GenshinColorGrading");
            _prop = new MaterialPropertyBlock();
            _pyramids = new Dictionary<int, GenshinColorGradingPyramid>();
        }
        ...
        public override void Render(CommandBuffer cmd, HDCamera camera, RTHandle srcRT, RTHandle destRT)
        {
            ...
            if (toneMap.value) _material.EnableKeyword("_CUSTOM_TONEMAP");
            else _material.DisableKeyword("_CUSTOM_TONEMAP");

            // シンプルにカラグレだけを実行
            _prop.SetTexture(ShaderIDs.InputTexture, srcRT);
    ...
    #region Image pyramid class used in GenshinColorGrading effect

    sealed class GenshinColorGradingPyramid
    {
        ...
        public GenshinColorGradingPyramid(HDCamera camera)
        {
            Allocate(camera);
        }
