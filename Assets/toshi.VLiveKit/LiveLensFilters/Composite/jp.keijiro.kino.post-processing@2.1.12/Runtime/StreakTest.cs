...
{
    [Serializable, VolumeComponentMenu("Post-processing/Kino/StreakTest")]
    public sealed class StreakTest : CustomPostProcessVolumeComponent, IPostProcessComponent
    {
        ...
        Dictionary<int, StreakTestPyramid> _pyramids;

        StreakTestPyramid GetPyramid(HDCamera camera)
        {
            StreakTestPyramid candid;
            ...
            else
            {
                ...
                _pyramids[cameraID] = candid = new StreakTestPyramid(camera);
            }
        ...
        {
            _material = CoreUtils.CreateEngineMaterial("Hidden/Kino/PostProcess/StreakTest");
            _prop = new MaterialPropertyBlock();
            _pyramids = new Dictionary<int, StreakTestPyramid>();
        }
        ...
        public override void Render(CommandBuffer cmd, HDCamera camera, RTHandle srcRT, RTHandle destRT)
        {
            ...
            HDUtils.DrawFullScreen(cmd, _material, destRT, _prop, 0);
            // var lastRT = pyramid[0].down;

            // // Downsample
            // var level = 1;
            // for (; level < StreakTestPyramid.MaxMipLevel && pyramid[level].down != null; level++)
            // {
    ...
    sealed class StreakTestPyramid
    {
        ...
        public StreakTestPyramid(HDCamera camera)
        {
            Allocate(camera);
        }
