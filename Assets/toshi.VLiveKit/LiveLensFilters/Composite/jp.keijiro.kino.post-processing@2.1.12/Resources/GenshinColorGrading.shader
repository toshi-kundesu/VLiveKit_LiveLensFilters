Shader "Hidden/Kino/PostProcess/GenshinColorGrading"
{
    HLSLINCLUDE
    ...
    #pragma multi_compile_local _ _CUSTOM_TONEMAP   // ★追加
...
{
    uint2 ss = IN.texcoord * _ScreenSize.xy;
    float3 col = LOAD_TEXTURE2D_X(_SourceTexture, ss).rgb;

    // Exposure
    col *= _Exposure;

#ifdef _CUSTOM_TONEMAP
    // Hable-ish トーンマップ
    float3 c0 = (1.36 * col + 0.047) * col;
    float3 c1 = (0.93 * col + 0.56) * col + 0.14;
    col = saturate(c0 / c1);
#endif

    // Contrast (LogC 補間)
    float3 logc = LinearToLogC(col);
    logc = lerp(ACEScc_MIDGRAY, logc, _Contrast);
    col  = LogCToLinear(logc);
