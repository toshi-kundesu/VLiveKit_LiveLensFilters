Shader "Hidden/Kino/PostProcess/StreakTest"
{
    HLSLINCLUDE
    ...
    float4 FragmentPrefilter(Varyings input) : SV_Target
    {
        ...
        uint2 pixelXY = input.texcoord * _ScreenSize.xy;

        float z = LoadCameraDepth(pixelXY);

        float linerDepth = LinearEyeDepth(z, _ZBufferParams);
        float3 depth = float3(linerDepth, 0, 0);   // m 単位の線形深度
        // depth *= float3(1, 0, 0);
        float3 defaultDepth = float3(z, 0, 0);
        float3 c = (c0 + c1) / 2;
        ...
        // return float4(float3(cameraDepth01), 1);
    }
    ...
    float4 FragmentUpsample(Varyings input) : SV_Target
    {
        ...
        const float dy = _InputTexture_TexelSize.y * 1.5;
        ...
        float3 c0 = SAMPLE_TEXTURE2D(_InputTexture, s_linear_clamp_sampler, float2(u0, v0)).rgb;
        float3 c1 = SAMPLE_TEXTURE2D(_InputTexture, s_linear_clamp_sampler, float2(u1, v1)).rgb;
        float3 c2 = SAMPLE_TEXTURE2D(_InputTexture, s_linear_clamp_sampler, float2(u2, v2)).rgb;
    ...
    float4 FragmentComposition(Varyings input) : SV_Target
    {
        ...
        // float z = SAMPLE_TEXTURE2D(_CameraDepthTexture, sampler_CameraDepthTexture, input.texcoord.xy);
        // float3 z = LOAD_TEXTURE2D(_CameraDepthTexture, positionSS).rgb;
        // input.positionSS.xy は uint2 型で取得（HDRP の例）
        // uint2 pixelXY = uint2(input.positionSS.xy);
