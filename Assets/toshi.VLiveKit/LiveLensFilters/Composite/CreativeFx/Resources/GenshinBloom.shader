Shader "Hidden/toshi/LensFilters/GenshinBloom"
{
    ...
    {
        float pixelRadius = max(_BlurRadius, 0.0) * lerp(1.0, 2.0, saturate(_Stretch));
        float2 stepUV = float2(_PostProcessScreenSize.z * max(pixelRadius, 1.0), 0.0);

        float3 center = HighlightSample(uv);
        float3 sum = center * GaussianCenterWeight;
        sum += HighlightSample(uv + stepUV * GaussianNearOffset) * GaussianNearWeight;
        sum += HighlightSample(uv - stepUV * GaussianNearOffset) * GaussianNearWeight;
        sum += HighlightSample(uv + stepUV * GaussianMiddleOffset) * GaussianMiddleWeight;
        sum += HighlightSample(uv - stepUV * GaussianMiddleOffset) * GaussianMiddleWeight;
        sum += HighlightSample(uv + stepUV * GaussianFarOffset) * GaussianFarWeight;
        sum += HighlightSample(uv - stepUV * GaussianFarOffset) * GaussianFarWeight;
    ...
    {
        float pixelRadius = max(_BlurRadius, 0.0) * lerp(1.0, 0.75, saturate(_Stretch));
    ...
    float4 FragmentComposite(Varyings input) : SV_Target
    {
        ...
        float3 bloom = BlurVertical(input.texcoord);
